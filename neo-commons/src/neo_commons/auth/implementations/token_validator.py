...
class DualStrategyTokenValidator:
    ...
    async def validate_token(
        ...
        """
        # Force introspection for critical operations (matching source)
        if critical:
            strategy = ValidationStrategy.INTROSPECTION
        
        # Check cache first
        token_hash = self._hash_token(token)
        cache_key = self.cache_keys.get_introspection_cache_key(token_hash)
        ...
        try:
            if strategy == ValidationStrategy.LOCAL:
                # Fast path: Local JWT validation only (matching source)
                ...
                # Secure path: Server-side introspection only (matching source)
                ...
                # Dual validation: Try local first, then introspection (matching source)
                try:
                    claims = await self._validate_local(token, realm)
                    validation_metadata["method"] = "local"
            ...
            # Add validation metadata to claims (matching source)
            claims["_validation"] = validation_metadata
            
            # Cache the validated claims (matching source)
            if strategy != ValidationStrategy.LOCAL:
    ...
    async def _validate_local(
        ...
        """
        try:
            # Get public key (cached) (matching source)
            public_key = await self._get_public_key(realm)
            
            # Decode and validate token (matching source pattern)
            ...
            # Only add issuer if verification is enabled (matching source)
            ...
            # Add audience parameter only if audience verification is enabled (matching source)
            ...
            try:
                ...
                # Handle audience validation fallback (matching source pattern)
            ...
            # Check if token is not expired (matching source)
    ...
    async def _validate_introspection(
        ...
        """
        try:
            # Introspect token with Keycloak (matching source)
    ...
    async def _get_public_key(self, realm: str) -> str:
        ...
        """
        # Check cache (matching source)
        cache_key = self.cache_keys.get_public_key_cache_key(realm)
        cached_key = await self.cache.get(cache_key)
        
        if cached_key:
            return cached_key
        
        # Get from Keycloak (matching source)
        public_key = await self.keycloak_client.get_realm_public_key(realm)
        
        # Cache the key (matching source)
        await self.cache.set(
            cache_key,
            public_key,
    ...
    async def is_token_revoked(self, token: str) -> bool:
        ...
        """
        token_hash = self._hash_token(token)
        revocation_key = self.cache_keys.get_revocation_cache_key(token_hash)
    ...
    async def revoke_token(
        ...
        """
        try:
            # Add token hash to revocation list in cache (matching source)
            token_hash = self._hash_token(token)
            revocation_key = self.cache_keys.get_revocation_cache_key(token_hash)
            
            # Get token expiry (matching source)
            import jwt as jose_jwt
            claims = jose_jwt.decode(token, options={"verify_signature": False})
            exp = claims.get("exp", 0)
            ttl = max(0, int(exp - datetime.utcnow().timestamp()))
            
            # Add to revocation list with TTL until token natural expiry (matching source)
            await self.cache.set(revocation_key, True, ttl=ttl)
            
            # Clear any cached validations (matching source)
            cache_key = self.cache_keys.get_introspection_cache_key(token_hash)
    ...
    async def refresh_if_needed(
        ...
        """
        try:
            # Decode token without validation to check expiry (matching source)
            import jwt as jose_jwt
            claims = jose_jwt.decode(token, options={"verify_signature": False})
            
            # Check if token expires soon (matching source)
            exp = claims.get("exp", 0)
            time_to_expiry = exp - datetime.utcnow().timestamp()
            
            if time_to_expiry < self.REFRESH_THRESHOLD:
                # Token expires soon, refresh it (matching source)
    ...
    async def clear_user_tokens(self, user_id: str) -> int:
        ...
        """
        # Use cache key provider pattern (matching source pattern but with provider)
