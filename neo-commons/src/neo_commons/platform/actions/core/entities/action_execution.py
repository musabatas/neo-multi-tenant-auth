"""Action execution entity for platform actions infrastructure.

This module defines the ActionExecution entity that represents a single execution
of an action with tracking, results, and performance metrics.

Moved from platform/events to platform/actions following maximum separation principle.
"""
...
class ActionExecution:
    ...
    Represents a single execution of an action with complete lifecycle tracking,
    ...
    id: ActionExecutionId = field(default_factory=ActionExecutionId.generate)
    action_id: ActionId = field(default_factory=ActionId.generate)
    
    # Event context (optional - actions can be executed independently)
    event_id: Optional[str] = None  # Generic event ID reference
    event_type: str = ""
    ...
    @classmethod
    def create_new(cls, action_id: ActionId, event_id: Optional[str] = None, 
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ActionExecution":
        ...
        execution_id = ActionExecutionId(UUID(data["id"]))
