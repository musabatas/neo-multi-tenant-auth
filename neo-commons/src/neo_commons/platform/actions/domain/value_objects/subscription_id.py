"""Event Action Subscription ID value object."""
...
class SubscriptionId:
    ...
    Represents a unique identifier for an event-action subscription using UUIDv7.
    ...
    def __post_init__(self):
        """Validate subscription ID on creation."""
        if not isinstance(self.value, UUID):
            raise TypeError(f"SubscriptionId value must be UUID, got {type(self.value)}")
    ...
    def generate(cls) -> 'SubscriptionId':
        """Generate a new SubscriptionId using UUIDv7."""
        return cls(generate_uuid_v7())
    
    @classmethod
    def from_string(cls, uuid_str: str) -> 'SubscriptionId':
        """Create SubscriptionId from UUID string."""
        try:
    ...
    def __eq__(self, other) -> bool:
        ...
        if isinstance(other, SubscriptionId):
