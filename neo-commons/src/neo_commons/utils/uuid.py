...
UUIDv7 provides time-ordered identifiers with better database performance
compared to UUIDv4. This module implements the UUIDv7 specification with
high-performance generation and timestamp extraction capabilities.

Key Benefits:
- Time-ordered for better database indexing
- Millisecond precision timestamps
- Uniqueness guarantees across distributed systems
- Backward compatible with standard UUID libraries

Usage:
    >>> from neo_commons.utils.uuid import generate_uuid_v7
    >>> uuid_str = generate_uuid_v7()
    >>> print(uuid_str)  # '01234567-89ab-7def-8123-456789abcdef'
"""
...
def extract_timestamp_from_uuid_v7(uuid_str: str) -> Optional[float]:
    ...
    Example:
        >>> uuid_str = generate_uuid_v7()
        >>> timestamp = extract_timestamp_from_uuid_v7(uuid_str)
...
def is_uuid_v7(uuid_str: str) -> bool:
    ...
    Example:
        ...
        >>> is_uuid_v7(uuid_str)
        True
        >>> is_uuid_v7("invalid-uuid")
        False
        >>> is_uuid_v7(str(uuid.uuid4()))  # UUIDv4
        False
    """
    try:
        uuid_obj = uuid.UUID(uuid_str)
...
def is_valid_uuid(uuid_str: str) -> bool:
    ...
    Example:
        >>> is_valid_uuid(generate_uuid_v7())
        True
...
generate_uuid = generate_uuid_v7  # Default to UUIDv7
