...
from ...exceptions import NotFoundError, ConflictError, ExternalServiceError
...
class DatabaseRealmManager:
    ...
    async def get_realm_for_tenant(
        ...
        """
        # Check cache first (matching source)
        if use_cache:
            cache_key = self.cache_keys.get_realm_cache_key(tenant_id)
            cached_realm = await self.cache.get(cache_key)
            if cached_realm:
                logger.debug(f"Cache hit for tenant {tenant_id} realm: {cached_realm}")
                return cached_realm
        
        # Query database (matching source pattern - CRITICAL: reads from external_auth_realm)
        query = """
            SELECT 
                external_auth_realm,
                name,
        ...
        if not realm_name:
            # Tenant doesn't have a realm configured yet (matching source)
        ...
        # Cache the realm name (matching source)
        if use_cache:
    ...
    async def ensure_realm_exists(
        ...
        """
        try:
            # Try to create the realm (matching source)
            ...
            # Apply additional settings if provided (matching source)
            if settings:
            ...
            # Realm already exists (matching source)
    ...
    async def configure_realm_client(
        ...
        """
        # Build client configuration (matching source)
        client_config = {
            "clientId": client_id,
        ...
        # Apply additional settings (matching source)
        if settings:
            client_config.update(settings)
        
        # Note: Actual client creation would be done through Keycloak Admin API
        # This is a placeholder for the client configuration (matching source)
    ...
    async def update_realm_settings(
        ...
        """
        # Note: This would use Keycloak Admin API to update realm settings (matching source)
    ...
    async def create_tenant_realm(
        ...
        """
        # Ensure realm exists (matching source)
        ...
        # Configure default client for the tenant (matching source)
        ...
        # Create admin user for the tenant (matching source)
        if not admin_password:
            # Generate secure password if not provided (matching source)
        ...
        # Update tenant in database with realm name (matching source)
        update_query = """
        ...
        # Clear cache for this tenant (matching source)
        cache_key = self.cache_keys.get_realm_cache_key(tenant_id)
        ...
        return {
            ...
            "admin_password": admin_password,  # Only returned on creation (matching source)
