...
Generic database utilities that can be used across all platform services
...
def process_database_record(
    ...
    Args:
        ...
        list_jsonb_fields: List of field names that contain JSONB lists (if None, auto-detects)
    ...
    if uuid_fields is None:
        # Auto-detect common UUID fields across platform services
        uuid_fields = [
            'id', 'region_id', 'organization_id', 'tenant_id', 'user_id', 
            'created_by', 'updated_by', 'deleted_by', 'parent_id',
            'role_id', 'permission_id', 'team_id', 'subscription_id'
        ]
    
    if jsonb_fields is None:
        # Auto-detect common JSONB fields across platform services
        jsonb_fields = [
            'metadata', 'config', 'settings', 'permissions', 'tags_metadata',
            'auth_config', 'cache_config', 'database_config', 'ui_settings'
        ]
    ...
    if list_jsonb_fields is None:
        ...
        list_jsonb_fields = [
            ...
            'backup_endpoints', 'features', 'roles', 'permissions_list'
        ]
...
def build_filter_conditions(
    ...
    Generic filter builder that supports common query operations
    across all platform database operations.
