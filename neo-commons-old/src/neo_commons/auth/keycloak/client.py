...
Enterprise-grade Keycloak client with async operations, multi-realm support,
...
from .protocols import KeycloakClientProtocol
...
class KeycloakClient:
    ...
    - Integration with python-keycloak async methods
    """
    ...
    def __init__(
        ...
        self.default_realm = default_realm or config.default_realm
        
        # Cache configurations from config
        self.TOKEN_CACHE_TTL = config.cache_ttl_tokens
        self.INTROSPECTION_CACHE_TTL = 60  # 1 minute
        self.PUBLIC_KEY_CACHE_TTL = config.cache_ttl_realm_keys
        ...
        logger.info("Initialized KeycloakClient with protocol-based configuration")
    ...
    def _map_keycloak_error(self, error: Exception, operation: str) -> Exception:
        ...
        if isinstance(error, KeycloakAuthenticationError):
            return AuthenticationError(f"{operation}: {str(error)}")
        elif isinstance(error, KeycloakGetError):
            if "404" in str(error):
                return UserNotFoundError("Resource not found")
    ...
    async def authenticate(
        ...
        try:
            ...
            # Use manual HTTP request for precise control over request formatting
            request_data = {
                "grant_type": "password",
                "client_id": effective_client_id,
    ...
    async def decode_token(
        ...
        """
        Decode and optionally validate a JWT token.
        ...
        try:
            ...
            if "audience" in error_str and "invalid" in error_str:
                ...
                # Try fallback decode without validation
                try:
                    decoded = await client.a_decode_token(
                        token,
                        validate=False
    ...
    async def create_realm(
        ...
        try:
            # Create realm using async method
