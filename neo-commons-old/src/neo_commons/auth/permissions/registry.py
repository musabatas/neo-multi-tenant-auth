"""
Permission Registry for Neo-Commons
...
class DefaultPermissionRegistry:
    """
    Default implementation of permission registry.
    ...
    def __init__(self):
        ...
        logger.info(f"Permission registry initialized with {len(self._permissions)} permissions")
    ...
    async def expand_permission_group(self, group_name: str) -> List[str]:
        ...
        if group_name not in self._groups:
            logger.warning(f"Permission group '{group_name}' not found")
            return []
    ...
    async def register_dynamic_permission(self, permission: PermissionDefinition) -> bool:
        ...
        if permission.code in self._permissions:
            logger.warning(f"Permission '{permission.code}' already exists")
            return False
        
        self._permissions[permission.code] = permission
        logger.info(f"Registered dynamic permission: {permission.code}")
        return True
...
def get_permission_registry() -> DefaultPermissionRegistry:
    """
    Get the global permission registry instance.
    
    Returns:
        Global DefaultPermissionRegistry instance
    """
    global _permission_registry
    if _permission_registry is None:
        _permission_registry = DefaultPermissionRegistry()
