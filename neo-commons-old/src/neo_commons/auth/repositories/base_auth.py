...
class BaseAuthRepository(BaseRepository[Dict[str, Any]]):
    ...
    async def get_user_roles(
        ...
        Args:
            user_id: User UUID
            tenant_id: Optional tenant scope (not used for platform roles)
            
        Returns:
            List of user roles with details
        """
        # Query to get platform roles (from admin.platform_user_roles)
        # Note: platform_user_roles doesn't have tenant_id - these are platform-level roles
        query = f"""
            SELECT 
                r.id,
                r.code,
                ...
                ur.expires_at
            FROM {self.schema_name}.platform_user_roles ur
            JOIN {self.schema_name}.platform_roles r ON r.id = ur.role_id
            WHERE ur.user_id = $1
        ...
        for record in results:
            roles.append(process_database_record(
                record,
                uuid_fields=['id', 'granted_by'],
                jsonb_fields=['role_config', 'metadata']
            ))
