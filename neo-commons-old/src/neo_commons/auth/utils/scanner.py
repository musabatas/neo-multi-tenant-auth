"""
Permission scanner for discovering endpoint permissions in Neo-Commons.
"""
...
from ..fastapi.decorators import PermissionMetadata
...
class EndpointPermissionScanner:
    ...
    def get_permissions_for_sync(self) -> List[Dict[str, Any]]:
        """
        Get permissions in format suitable for database synchronization.
        
        Returns:
            List of permission definitions for database insert/update
        """
        permissions = []
        for perm in self.discovered_permissions.values():
            permissions.append({
                ...
                'is_active': True,
                'is_system': True  # Mark as system-discovered
            })
        return permissions


# Factory function for creating scanner
def create_permission_scanner(app: FastAPI) -> EndpointPermissionScanner:
    """
    Create an endpoint permission scanner.
    
    Args:
        app: FastAPI application instance
        
    Returns:
        Configured EndpointPermissionScanner instance
    """
    return EndpointPermissionScanner(app)
