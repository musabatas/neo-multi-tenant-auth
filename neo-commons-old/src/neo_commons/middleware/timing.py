...
Generic timing middleware that can be used across all platform services
in the NeoMultiTenant ecosystem.
"""
import time
import re
from typing import Callable, Dict, Any, Optional, List, Protocol, runtime_checkable
from fastapi import Request, Response
from starlette.middleware.base import BaseHTTPMiddleware
from loguru import logger

from neo_commons.middleware.logging import get_request_context


@runtime_checkable
class TimingConfig(Protocol):
    """Protocol for timing configuration."""
    
    @property
    def add_timing_header(self) -> bool:
        """Whether to add timing headers to responses."""
        ...
    ...
    def slow_request_threshold(self) -> float:
        """Threshold in seconds for slow request detection."""
        ...
    
    @property
    def very_slow_threshold(self) -> float:
        """Threshold in seconds for very slow request detection."""
        ...
...
class TimingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        if config:
            self.add_timing_header = config.add_timing_header
            self.log_slow_requests = config.log_slow_requests
            self.slow_request_threshold = config.slow_request_threshold
            self.very_slow_threshold = config.very_slow_threshold
            self.exclude_paths = config.exclude_paths
            self.track_detailed_timing = config.track_detailed_timing
        else:
    ...
    async def _log_slow_request(
        ...
        getattr(logger, log_level)("Slow request detected", **log_data)
...
class ResponseSizeMiddleware(BaseHTTPMiddleware):
    ...
    async def _log_large_response(
        ...
        logger.warning("Large response detected", **log_data)
