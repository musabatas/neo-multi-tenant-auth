-- V1013__create_event_actions_infrastructure.sql
-- Create tables and infrastructure for dynamic event actions system
-- This enables flexible, configurable actions triggered by domain events

-- =====================================================================================
-- EVENT ACTIONS TABLE
-- Core registry for configurable event actions
-- =====================================================================================
CREATE TABLE IF NOT EXISTS admin.event_actions (
    ...
    tenant_id UUID, -- Optional tenant filtering
...
CREATE INDEX IF NOT EXISTS idx_event_actions_handler_type ON admin.event_actions(handler_type);
CREATE INDEX IF NOT EXISTS idx_event_actions_status_enabled ON admin.event_actions(status, is_enabled);
CREATE INDEX IF NOT EXISTS idx_event_actions_tenant_id ON admin.event_actions(tenant_id) WHERE tenant_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_event_actions_created_by ON admin.event_actions(created_by_user_id);
CREATE INDEX IF NOT EXISTS idx_event_actions_updated_at ON admin.event_actions(updated_at);

-- GIN indexes for JSONB fields for efficient filtering
CREATE INDEX IF NOT EXISTS idx_event_actions_event_types_gin ON admin.event_actions USING GIN(event_types);
CREATE INDEX IF NOT EXISTS idx_event_actions_conditions_gin ON admin.event_actions USING GIN(conditions);
CREATE INDEX IF NOT EXISTS idx_event_actions_context_filters_gin ON admin.event_actions USING GIN(context_filters);
CREATE INDEX IF NOT EXISTS idx_event_actions_tags_gin ON admin.event_actions USING GIN(tags);

-- =====================================================================================
-- ACTION EXECUTIONS TABLE  
-- Log of action execution attempts with results
-- =====================================================================================
CREATE TABLE IF NOT EXISTS admin.action_executions (
    id UUID PRIMARY KEY DEFAULT platform_common.uuid_generate_v7(),
    action_id UUID NOT NULL REFERENCES admin.event_actions(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_action_executions_action_id ON admin.action_executions(action_id);
CREATE INDEX IF NOT EXISTS idx_action_executions_status ON admin.action_executions(status);
CREATE INDEX IF NOT EXISTS idx_action_executions_event_type ON admin.action_executions(event_type);
CREATE INDEX IF NOT EXISTS idx_action_executions_created_at ON admin.action_executions(created_at);
CREATE INDEX IF NOT EXISTS idx_action_executions_completed_at ON admin.action_executions(completed_at);

-- Index for finding failed executions to retry
CREATE INDEX IF NOT EXISTS idx_action_executions_retry_lookup ON admin.action_executions(status, retry_count, created_at) 
...
DROP TRIGGER IF EXISTS trigger_update_event_actions_updated_at ON admin.event_actions;
CREATE TRIGGER trigger_update_event_actions_updated_at
    BEFORE UPDATE ON admin.event_actions
    FOR EACH ROW
...
CREATE OR REPLACE FUNCTION validate_action_configuration()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trigger_validate_action_configuration ON admin.event_actions;
CREATE TRIGGER trigger_validate_action_configuration
    BEFORE INSERT OR UPDATE OF configuration, handler_type ON admin.event_actions
    FOR EACH ROW
    EXECUTE FUNCTION validate_action_configuration();
...
DROP TRIGGER IF EXISTS trigger_update_action_statistics ON admin.action_executions;
CREATE TRIGGER trigger_update_action_statistics
    AFTER INSERT OR UPDATE OF status ON admin.action_executions
    FOR EACH ROW
...
CREATE OR REPLACE VIEW admin.v_action_performance AS
SELECT 
    ea.id,
    ea.name,
    ea.handler_type,
...
FROM admin.event_actions ea
LEFT JOIN admin.action_executions ae ON ea.id = ae.action_id
...
CREATE OR REPLACE VIEW admin.v_execution_monitoring AS
SELECT 
...
FROM admin.action_executions ae
JOIN admin.event_actions ea ON ae.action_id = ea.id
...
-- Add comment describing the event actions system
COMMENT ON TABLE admin.event_actions IS 
'Event actions registry for dynamic event-driven automation. Enables configurable actions to be triggered by domain events.';

COMMENT ON TABLE admin.action_executions IS 
'Log of event action executions with results, errors, and performance metrics.';

-- Grant appropriate permissions (adjust based on your security model)
-- These would typically be configured based on your specific role structure

-- =====================================================================================
-- CLEANUP FUNCTIONS
-- =====================================================================================

-- Function to archive old executions (can be called by scheduled job)
CREATE OR REPLACE FUNCTION archive_old_action_executions(retention_days INTEGER DEFAULT 30)
RETURNS INTEGER AS $$
DECLARE
    deleted_count INTEGER;
BEGIN
    DELETE FROM admin.action_executions 
